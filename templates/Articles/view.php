...
<div style="max-width: 800px; margin: 0 auto; padding: 2rem;">
    ...
    <div style="background: #e3f2fd; border: 1px solid #2196f3; border-radius: 8px; padding: 1rem; margin-bottom: 1.5rem;">
        <h3 style="margin-top: 0; color: #1565c0;">Permission Check Results</h3>
        ...
        <p style="margin-bottom: 0.5rem;">
            ...
            | <strong>You Own This:</strong> <?= $isOwner ? '<span style="color: #2e7d32;">Yes</span>' : '<span style="color: #c62828;">No</span>' ?>
        </p>
    ...
    <div style="background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 1.5rem; margin-bottom: 1.5rem; <?= $isOwner ? 'border-left: 4px solid #4caf50;' : '' ?>">
        <h2 style="margin-top: 0;"><?= h($article->title) ?></h2>
    ...
    <div style="margin-bottom: 1.5rem;">
        <?php if ($canEdit) { ?>
            <?= $this->Html->link('Edit Article', ['action' => 'edit', $article->id], [
                'style' => 'display: inline-block; padding: 0.5rem 1rem; background: #2196f3; color: white; text-decoration: none; border-radius: 4px; margin-right: 0.5rem;',
            ]) ?>
        <?php } else { ?>
